...
function makeWorldStub() {
  const captured: { bodyDesc?: any; colliderDesc?: any; parentBody?: any } = {};
...
test('place le corps au centre fourni', () => {
  ...
  FlipperPhysics.createBody(stub.world as any, center, { x: 0.1, y: 0.02, z: 0.04 });
...
test('demi-extents X et Z = moitié de la taille, Z élargi du rayon de bille', () => {
  ...
  FlipperPhysics.createBody(stub.world as any, { x: 0, y: 0, z: 0 }, size);
...
test('applique restitution flipper, friction 0.28 et active les events de collision', () => {
  FlipperPhysics.createBody(stub.world as any, { x: 0, y: 0, z: 0 }, { x: 0.1, y: 0.04, z: 0.04 });
