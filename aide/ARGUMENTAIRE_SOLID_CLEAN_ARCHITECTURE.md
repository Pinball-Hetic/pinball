...
**Exemple de diagnostic SRP confirmé après coup** (bon angle si le jury demande "sais-tu repérer une
violation de SRP") : une version antérieure de `CollisionEventProcessor` gérait à la fois le dispatch
générique ET la logique du monde alternatif (baselines de score, flag actif/inactif) directement dans ses
propres champs — un vrai mélange de deux responsabilités. Ce diagnostic a depuis été confirmé par le code
lui-même : la logique du monde alternatif a été extraite dans une classe dédiée, `AlternateWorldState.ts`
(state holder pur). `CollisionEventProcessor` délègue maintenant à `this.worldState` au lieu de porter les
champs directement. Voir `PREPA_CODE_REVIEW_CollisionHandlers.md` §3 et §8 pour le détail complet.
...
**Preuve dans le code** : les 10 classes qui `implements CollisionHandler` sont toutes interchangeables dans
...
**Preuve dans le code** : `CollisionHandler` est l'interface commune, les 10 classes concrètes sont les
stratégies, `this.handlers.find(h => h.canHandle(role))` fait le choix au runtime, à chaque collision.
`BossCollisionHandler` en est le plus récent exemple : une logique auparavant en dur dans `process()` a été
extraite comme une stratégie de plus, simplement placée en tête du tableau pour la priorité.
...
| Adapter | `CollisionEventProcessor.ts`, `CollisionHandler.ts`, `AlternateWorldState.ts` | Traduisent l'event physique brut en appel de Use Case, ou portent un état de session pur |
| Infrastructure | `BumperCollisionHandler.ts` (partiellement — il touche `pendingPhysics`/Rapier indirectement), `BossCollisionHandler.ts`, `BallPhysics.ts` | Détails techniques concrets (Rapier, positions GLB) |

`AlternateWorldState.ts` est un bon exemple à citer hors des 4 fichiers présentés : c'est un state holder
pur (4 champs, une dizaine de méthodes), zéro dépendance à Rapier/Three.js — au sens strict, plus proche
d'un Adapter/Domain state que d'Infrastructure, malgré son emplacement dans le dossier `infrastructure/`.
Même nuance que pour `BumperCollisionHandler` plus bas : l'emplacement dans l'arborescence ne garantit pas
toujours la couche exacte, c'est le contenu qui compte.
